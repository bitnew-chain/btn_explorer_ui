<template>
    <div class="date-pickers">
        <input type="text" class="blockinput" id="datePickers" placeholder="选择日期" @focus="trueDateBox" :value="date" readonly/>
        <transition name="fade">
            <div class="date-box" v-if="dateBoxFlag">
                <div class="day-select">
                    <div>
                        <span @click="reduceYear" class="left">&laquo;</span>
                    ...
                    <div>
                        <span @click="addMonth" class="left">&gt;</span>
                        <span @click="addYear">&raquo;</span>
                    </div>
                </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            showAndHide(){
                ...
                document.addEventListener('click',function(event){
                    try{
                        let dateClass = document.getElementsByClassName('date-pickers')[0]
                        if(!dateClass.contains(event.target)){
                            that.dateBoxFlag = false;
                        }
                    }catch(error){
            ...
            addZero(value){
                let newValue = ''
                newValue = parseInt(value) < 10 ? '0'+ parseInt(value) : value;
                return newValue
            },
            // 增减年份
            addYear () {
        ...
        mounted(){
            setTimeout(()=>{
                this.showAndHide();
                let urlDate = location.search.split('=')[1];
                if(urlDate){
                    this.year = urlDate.split('-')[0];
                    this.month = urlDate.split('-')[1];
                    this.day = urlDate.split('-')[2];
                }
